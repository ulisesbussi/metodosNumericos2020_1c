...
%Descenso Thomas
    ...
    %Z(1) = (((-h^2)/k(X(1))*f1?) - yAux(1)));  
    %Acá tomé a f(X(1)) como Z(1) y despejé. L(i,i) = 1, se omitió
    
    Z(1) = -yAux(1) / (((h^2)/k(X(1)))+1);
    for i=2:n-1
        %Y(i) = (((-h^2)/k(X(i))*fi? - yAux(i)) - Y(i-1)*L(i,i-1));
        %Acá tomé a f(X(i)) como Z(i) y despejé
        
        Z(i) = (-yAux(i) - Z(i-1)*L(i,i-1)) / (((h^2)/k(X(i)))+1);
    end
